  ...
  if (el == 'outline') { // We have an outline node
    ...
    Object.keys(n).forEach(function(el){
      ...
      case('title'):
        ...
        break;
      case('datecreated'):
        self.meta.dateCreated = getValue(n[el]) ? new Date(n[el]['#']) : null;
        break;
      case('datemodified'):
        self.meta.dateModified = getValue(n[el]) ? new Date(n[el]['#']) : null;
        break;
      case('ownername'):
        self.meta.ownerName = getValue(n[el]);
        break;
      case('ownerid'):
        self.meta.ownerId = getValue(n[el]);
        break;
      case('docs'):
        self.meta.docs = getValue(n[el]);
        break;
      }
    });
    self.emit('meta', self.meta);
